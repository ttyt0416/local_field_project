...
이미지 상세 페이지는 진입할 때마다 `GET /vault/images/{generation_id}`를 호출하고 서버에서 조회수를 증가시킨다. Storage signed URL 또는 인증 이미지 프록시로 결과 이미지를 표시하며, 상단에는 `이미지 상세` 제목과 조회수, 즐겨찾기 버튼만 표시한다. 완료 상태의 `IMAGE · 완료` 텍스트와 별도 타입 아이콘 container는 표시하지 않는다.
