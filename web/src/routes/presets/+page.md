...
프리셋 관리는 visible `IMAGE`/`VIDEO` title 없이 최상단 `IMAGE`, `VIDEO` 2-tab을 표시한다. IMAGE가 선택되면 그 아래 `ANIMA`, `ILLUSTRIOUS`, `KREA2` family 3-tab과 `T2I`, `I2I` mode 2-tab을 동일한 간격으로 표시한다. 기본 선택은 `IMAGE` → `ANIMA` → `T2I`다. 새 프리셋 button은 이 tab cluster 바로 아래 우측에 둔다. 각 조합은 정확히 하나의 preset type만 조회·기본 지정·삭제한다. Krea2는 workflow와 model option contract가 아직 없어 새 preset 생성·수정은 disabled 상태다.
