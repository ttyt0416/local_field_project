...
프리셋 관리는 `IMAGE`와 `VIDEO`를 별도 title로 표시한다. `IMAGE` 아래에는 `ANIMA`, `ILLUSTRIOUS`, `KREA2` family tab과 그 아래 `T2I`, `I2I` mode tab을 둔다. 기본 선택은 `ANIMA`와 `T2I`다. 각 조합은 정확히 하나의 preset type만 조회·기본 지정·삭제한다. Krea2는 workflow와 model option contract가 아직 없어 새 preset 생성·수정은 disabled 상태다.
