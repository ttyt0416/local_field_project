...
동영상 생성의 image source selector는 `저장된 콘텐츠 → 생성 → T2I (Anima)`를 initial state로 열고, `T2I (Anima)`, `I2I (Anima)`, `T2I (Illustrious)`, `I2I (Illustrious)`, `T2I (Krea2)`, `I2I (Krea2)` 6-category grid를 표시한다. selected category는 `/uploads`의 `generation_mode`와 `model_family` exact filter로 전송한다. video/audio reference는 image generation category filter 없이 기존 device selection을 유지한다.
