...
`Checkpoint`는 current video mode의 `/generation/video/options` 결과를 folder-filtered modal로 표시한다. 선택하면 modal은 즉시 닫히고 Eros 또는 compatible MiniMax base가 request, preset, Vault detail에 같은 값으로 남는다. mode를 바꾸면 Eros는 유지되고 MiniMax는 새 mode의 compatible base로 기본값이 바뀐다.
