...
`image_generations.storage_file_id`와 `video_generations.storage_file_id`, `three_d_generations.storage_file_id`는 실제 바이트가 아니라 독립 Storage 서비스의 파일 식별자만 보관한다. `size_bytes`는 저장 결과의 실제 파일 용량을 별도로 보관한다. 이미지 row는 `model_family`, `generation_mode`, nullable `source_file_id`·`source_filename`, `denoise`를 저장해 Anima·Illustrious T2I/I2I를 같은 lifecycle로 관리한다. 3D row는 source image `source_file_id`와 결과 GLB `storage_file_id`를 분리하고, 결과 MIME은 `model/gltf-binary`로 고정한다.
