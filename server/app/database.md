...
`image_generations`와 `video_generations` 보관함 목록은 사용자·검색·정렬·favorite 필터를 DB에 적용하고 page size 10과 `LIMIT/OFFSET`으로 페이지를 반환한다. 목록 조회는 전체 개수와 완료 개수를 함께 계산해 API가 `total_count`, `completed_count`, `total_pages`를 제공한다.
