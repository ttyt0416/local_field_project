...
`generation_worker.py`는 서버 startup 시 실행되는 reconciler다. DB에서 `queued` 또는 `processing` 상태인 이미지·동영상·3D·음악 generation을 조회하고, 각 작업의 ComfyUI history를 확인한다.
...
- 음악 완료: ComfyUI audio output을 검증해 Storage에 저장하고 `music_generations`를 완료 상태로 갱신한다.
...
프런트는 모든 generation kind의 generation별 SSE endpoint를 구독한다. SSE는 연결 시 DB snapshot을 한 번 전송한 뒤 worker가 publish한 상태·완료·실패 이벤트를 받으며, 연결이 끊기면 재연결 후 DB snapshot으로 누락된 상태를 보완한다. 실제 inference와 video queue 실행은 이 계약 테스트에서 호출하지 않는다.
