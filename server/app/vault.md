...
프롬프트 검색은 `image_generations.prompt`에 대한 PostgreSQL `pg_trgm` GIN 인덱스를 사용한다. 삭제 API는 Storage 파일 삭제가 성공한 뒤 Local Field의 레코드를 삭제한다. Storage 삭제가 실패하면 레코드를 유지해 재시도할 수 있다.
