...
class GenerationWorkerTest(unittest.TestCase):
    def test_reconciler_processes_active_images_and_videos_and_publishes_changes(self) -> None:
        ...
        image_result = comfyui.ImageGenerationStatus(prompt_id="image-prompt", status="processing")
        video_result = video.VideoGenerationStatus(prompt_id="video-prompt", mode="i2v", status="processing")
        with (
